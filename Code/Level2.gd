...
## Fase 2: Caminho para a Torre do Palácio (Combate)
## Odisseu precisa lutar contra a horda de guardas nas ruas estreitas.
...
func _build_background() -> void:
	...
	bg.color = Color(0.1, 0.05, 0.05) # Fundo mais avermelhado para clima de guerra
...
func _build_allies() -> void:
	...
	for i in range(20):
		...
		# Nascem amontoados perto do Odisseu
...
func _on_health_changed(hp: int) -> void:
	for i in range(10):
		if i < hp:
			hp_icons[i].color = Color(1.0, 0.2, 0.2) # Cheio
		else:
			hp_icons[i].color = Color(0.2, 0.2, 0.2) # Vazio
...
func _on_tower_entered(body: Node) -> void:
	if body == player and not GameManager.game_over:
		...
		t.timeout.connect(func(): get_tree().change_scene_to_file("res://Estoria/Ato1/Level3Cutscene.tscn"))
